...
ViewStation::ViewStation(std::string level, unsigned levelNum)
	:m_view(V2f{ 0,0 },windowSize ), m_gameOver(false), m_level(levelNum),
	m_astronaut(std::make_shared<Astronaut>(V2f{ 1,1 }, V2f{ 0,100 })),
	m_bg(std::make_unique<Background>()),
    m_info(std::make_unique<InfoBar>(V2f{ 0,0 }, V2f{ 1200, 60 }, m_level, 0, 5, sf::Time{ sf::seconds(300.0f) }))

{
	...
	m_background.setTexture(&m_bgtxt);
	...
	m_windowRec = sf::FloatRect{ 0,0,windowSize.x+150,windowSize.y+150 };
...
void ViewStation::drawView(sf::RenderWindow & myWindow)
{
	...
	m_view.setCenter(m_astronaut->getPosition());
	myWindow.setView(m_view);
	m_info->setScore(m_astronaut->getFruitsNum() * 2);
	m_info->setLive(m_astronaut->getLifesNum());
	
	m_background.setPosition(m_astronaut->getPosition() - windowSize / 2);//#
	
	m_bg->setPosition(m_astronaut->getPosition() - windowSize / 2) ;
	m_windowRec.left = m_background.getPosition().x-150;
	m_windowRec.top = m_background.getPosition().y-150;
	...
	for (auto &i : m_graph)
	{
		...
		if(m_windowRec.intersects(i.second->getGlobalBounds()))
		  i.second->draw(myWindow);
	}  
...
void ViewStation::updateMove(Movments movmentMode, Movments side2move)
{
	...
	m_info->setPosition(m_astronaut->getPosition() - V2f{ 480, 350 });
	m_gameOver = m_astronaut->getLifesNum() < 0 ; 
...
void ViewStation::updateCharsMap()
{
	...
	if (updated) return;*/
	m_charsMap['H'] = [&](V2f & p) { auto obj = getNewObj("Alien", 3, V2f{ .10f,.15f }, V2f{ p.x * 45 - 600 ,p.y * 55 - 41 }); m_graph.push_back(obj);	m_movableVec.push_back(obj.second);	};
